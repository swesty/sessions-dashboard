...
function pick(
  ...
  const r = labelMatch
    ? results.find((r) =>
        Object.entries(labelMatch).every(([k, v]) => r.metric[k] === v)
      )
    : results[0];
...
const GPU_HOSTS: GpuHostDef[] = [
  {
    ...
    model: 'NVIDIA GB10',
    unified: true,
    utilLabel: { host: 'spark-2' },
    tempLabel: { host: 'spark-2' },
    powerLabel: { host: 'spark-2' },
    vramLabel: null,
    maxPower: 250,
  },
  {
    ...
    model: 'RTX PRO 6000',
    unified: false,
    utilLabel: { host: 'jarvis' },
    tempLabel: { host: 'jarvis' },
    powerLabel: { host: 'jarvis' },
    vramLabel: { host: 'jarvis' },
    maxPower: 300,
  },
  {
    key: 'spark-1',
    ...
    model: 'NVIDIA GB10',
    unified: true,
    utilLabel: { name: 'NVIDIA_GB10' },
    tempLabel: { name: 'NVIDIA_GB10' },
    powerLabel: { name: 'NVIDIA_GB10' },
    vramLabel: null,
    maxPower: 250,
  },
  {
    ...
    model: 'RTX 3090',
    unified: false,
    utilLabel: { host: 'gesserit' },
    tempLabel: { host: 'gesserit' },
    powerLabel: { host: 'gesserit' },
    vramLabel: { host: 'gesserit' },
    maxPower: 350,
  },
];
...
async function fetchGPU() {
  const [util, util1, temp, temp1, power, power1, vramUsed, vramFree] =
    await Promise.all([
      instant('DCGM_FI_DEV_GPU_UTIL'),
      instant('nvidia_gpu_utilization_percent'),
      instant('DCGM_FI_DEV_GPU_TEMP'),
      instant('nvidia_gpu_temperature_celsius'),
      instant('DCGM_FI_DEV_POWER_USAGE'),
      instant('nvidia_gpu_power_draw_watts'),
      instant('DCGM_FI_DEV_FB_USED'),
      instant('DCGM_FI_DEV_FB_FREE'),
    ]);
  return {
    util: [...util, ...util1],
    temp: [...temp, ...temp1],
...
async function fetchSys() {
  const [mem, cpu] = await Promise.all([
    instant(
      '(node_memory_MemAvailable_bytes or (node_memory_free_bytes + node_memory_inactive_bytes)) / 1024 / 1024 / 1024'
    ),
    instant(
      '(1 - avg by (host) (rate(node_cpu_seconds_total{mode="idle"}[1m]))) * 100'
    ),
  ]);
  return { mem, cpu };
}
...
async function fetchLLM() {
  const [running, waiting, rate, spend] = await Promise.all([
    ...
    instant('round(rate(litellm_proxy_total_requests_metric_total[5m]) * 300)'),
...
function VramBar({ used, free }: { used: number | null; free: number | null }) {
  ...
  const color =
    pct >= 95 ? 'bg-red-500' : pct >= 80 ? 'bg-amber-500' : 'bg-cyan-500';
  return (
    <div className='w-full'>
      <div className='flex justify-between text-[10px] text-zinc-400 mb-1'>
        <span>VRAM</span>
        <span>
          {usedGb.toFixed(1)}/{totalGb.toFixed(0)} GiB
        </span>
      </div>
      ...
      <div className='flex items-start gap-4'>
        <div className='flex gap-3'>
          ...
          <CircularGauge
            ...
            label='Power'
            unit='W'
            warn={host.maxPower * 0.75}
            crit={host.maxPower * 0.9}
            size={88}
          />
          <CircularGauge
            value={temp}
        ...
        <div className='flex-1 min-w-0 space-y-2 pt-1'>
          ...
          <StatRow
            label='Power draw'
            value={power !== null ? power.toFixed(0) : null}
            unit='W'
          />
          <StatRow
            label='Temp'
            value={temp !== null ? temp.toFixed(0) : null}
            unit='°C'
          />
        </div>
      </div>
...
export function MetricsPanel() {
  ...
  function SectionHeading({ children }: { children: React.ReactNode }) {
    return (
      <h3 className='text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-3 mt-6 first:mt-0'>
        {children}
      </h3>
    );
  }
  ...
  return (
    <div>
      ...
      {gpuLoading ? (
        ...
        <div className='grid grid-cols-1 xl:grid-cols-2 gap-3'>
          {GPU_HOSTS.map((h) => (
            <GpuHostCard key={h.key} host={h} gpu={gpu!} />
          ))}
        </div>
      )}
            ...
            >
              ...
              <div className={`text-base font-semibold tabular-nums ${color}`}>
                {v !== null ? (
                  <>
                    {v.toFixed(1)}
                    <span className='text-[10px] text-zinc-500 ml-0.5'>GiB</span>
                  </>
                ) : (
                  <span className='text-zinc-600'>—</span>
                )}
